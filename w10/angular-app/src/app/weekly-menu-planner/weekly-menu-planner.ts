...
export class WeeklyMenuPlannerComponent implements OnInit {
  ...
  openDatePicker(): void {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      const date = this._parseToDate(result);
      if (date) {
  ...
  handleCalendarSelect(event: any): void {
    const date = this._parseToDate(event);
    if (date) this.currentDialogRef?.close(date);
  }
  ...
  private _parseToDate(input: any): Date | null {
    if (!input && input !== 0) return null;

    // Direct Date
    if (input instanceof Date && !isNaN(input.getTime())) return input;

    // MatDatepicker / MatInput event shapes: { value: Date }
    if (input && typeof input === 'object') {
      // unwrap common wrappers
      if (input.value instanceof Date && !isNaN(input.value.getTime())) return input.value;
      if (input.selected instanceof Date && !isNaN(input.selected.getTime())) return input.selected;
      // Moment-like object with toDate()
      ...
      // Plain object with year/month/day (e.g., {year:2026, month:4, day:14})
      ...
      // nested value
      if (input.value) return this._parseToDate(input.value);
    }
    ...
    // If input is a string, try Date parse
