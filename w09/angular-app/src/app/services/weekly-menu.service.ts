...
export class WeeklyMenuService {
  private readonly collectionName = 'weekly-menu';
  ...
  constructor(private recipeService: RecipeService, private toast: ToastService, private auth: AuthService) {}
  ...
  async loadWeeklyMenu(path = '/assets/data/weekly-menu.json', weekStart?: string) {
    ...
    try {
          ...
          if (ownerId) {
            ...
            if (!snap.empty) {
              ...
              const menuFromFs: WeeklyMenu = {
                id: docSnap.id,
                ownerId: m.ownerId || m.userId || ownerId,
      ...
      const pickMenu = (m: any) => ({
        id: m.id,
        ownerId: m.ownerId || m.userId || 'unknown',
      ...
      if (!menu && weekStart) {
        const ownerId = this.auth.uid() || 'unknown';
  ...
  private async _saveToFirestore(menu: WeeklyMenu): Promise<string | null> {
    const ownerId = this.auth.uid();
    if (!ownerId) throw new Error('A heti menü mentéséhez be kell jelentkezni.');

    if (menu.id && !menu.id.startsWith('menu_empty_')) {
      const ref = doc(db, this.collectionName, menu.id);
      await updateDoc(ref, { ownerId, slots: menu.slots, updatedAt: serverTimestamp() });
      return menu.id;
    }
