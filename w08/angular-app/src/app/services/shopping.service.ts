...
export class ShoppingService {
  ...
  constructor(private http: HttpClient, private weeklyMenuService: WeeklyMenuService, private recipeService: RecipeService, private toast: ToastService) {
  ...
  private async _loadCurrentWeek(): Promise<void> {
    try {
      ...
      const col = collection(db, 'shopping-list');
      const q = query(col, where('weekOf', '==', weekKey));
  ...
  async generateList(weeklyMenuId?: string): Promise<void> {
    try {
      ...
      if (weeklyMenuId && (!menu || menu.id !== weeklyMenuId)) {
        try {
          const ref = doc(db, 'weekly-menu', weeklyMenuId);
      ...
      if (!menu) {
        ...
        try {
          ...
          const qWeek = query(collection(db, 'weekly-menu'), where('weekStart', '==', weekStartKey));
      ...
      // 7) save to Firestore (collection: shopping-list)
      const col = collection(db, 'shopping-list');
      ...
      if (snap && !snap.empty) {
        ...
        const docSnap = snap.docs[0];
        await setDoc(doc(db, 'shopping-list', docSnap.id), payload);
        listId = docSnap.id;
      } else {
  ...
  private async _saveToFirestore(list: ShoppingList): Promise<string | null> {
    ...
    if (list.id) {
      try {
        const ref = doc(db, 'shopping-list', list.id);
        const snap = await getDoc(ref);
        if (snap.exists()) {
          await updateDoc(ref, { items: list.items, updatedAt: serverTimestamp() });
          return list.id;
        }
      } catch (err) {
    ...
    if (list.weekOf) {
      const q = query(col, where('weekOf', '==', list.weekOf));
      existingSnap = await getDocs(q);
    }
    if (existingSnap && !existingSnap.empty) {
      const docSnap = existingSnap.docs[0];
      const ref = doc(db, 'shopping-list', docSnap.id);
      await updateDoc(ref, { items: list.items, updatedAt: serverTimestamp() });
      return docSnap.id;
    }
